...
update_generic = DummyUpdate(
    chat=DummyChat("609"), from_user=DummyUser("1377", "kurkkumopo")
)
...
class TestCoreFunctions(unittest.TestCase):
    def test_get_ids(self):
        update = DummyUpdate(
            chat=DummyChat(123123), from_user=DummyUser(456456, "moippa")
        )
...
class TestSensorReading(unittest.TestCase):
    def test_sensor_messages(self):
        ...
        sensor_light = cr.handle_sensor(
            {"value": 255, "inserted": "2021-02-08T14:12:10.935Z", "sensor": "light1"}
        )
        sensor_voice = cr.handle_sensor(
            {"value": 1, "inserted": "2021-02-08T14:12:10.935Z", "sensor": "voice1"}
        )

        self.assertEqual(
            "Someone is in the darkroom 😊", cr.get_light_message(sensor_light)
        )
        self.assertEqual(
            "Somebody is in the virtual darkroom 😊", cr.get_voice_message(sensor_voice)
        )

        sensor_light = cr.handle_sensor(
            {"value": 0, "inserted": "2021-02-08T14:12:10.935Z", "sensor": "light1"}
        )
        sensor_voice = cr.handle_sensor(
            {"value": 0, "inserted": "2021-02-08T14:12:10.935Z", "sensor": "voice1"}
        )

        self.assertEqual("Darkroom is empty ☹️", cr.get_light_message(sensor_light))
        self.assertEqual(
            "Virtual darkroom is empty ☹️", cr.get_voice_message(sensor_voice)
        )
